...
// (wjp) 配置修改
std::string my_stat_output_file = "../a_my_log/10GB_EBS_test/5G_my_stat.log";
std::string rocksdb_stat_output_file = "../a_my_log/10GB_EBS_test/5G_rocks_stat.log";
std::string target_disk_name1 = "nvme2n1";
std::string target_disk_name2 = "nvme2n1p2";
...
void print_summary(rocksdb::DB* db, std::ofstream& outFile) {
  uint64_t bytes_read =
      db->GetOptions().statistics->getTickerCount(BYTES_READ);
  uint64_t bytes_write =
      db->GetOptions().statistics->getTickerCount(BYTES_WRITTEN);
  uint64_t number_keys_write =
      db->GetOptions().statistics->getTickerCount(NUMBER_KEYS_WRITTEN);
  uint64_t number_keys_read =
      db->GetOptions().statistics->getTickerCount(NUMBER_KEYS_READ);
  uint64_t number_db_seek =
      db->GetOptions().statistics->getTickerCount(NUMBER_DB_SEEK_FOUND);
  uint64_t number_db_next =
      db->GetOptions().statistics->getTickerCount(NUMBER_DB_NEXT_FOUND);
  uint64_t number_db_prev =
      db->GetOptions().statistics->getTickerCount(NUMBER_DB_PREV_FOUND);
  uint64_t iter_bytes_read =
      db->GetOptions().statistics->getTickerCount(ITER_BYTES_READ);
  auto writer_stall =
      db->GetOptions().statistics->getTickerCount(STALL_MICROS);

  outFile << "***my summary***" << endl;
  outFile << "total run time: " << total_time << "s" << endl;
  outFile << "user read size: " << bytes_read * 1.0 / kMB
          << "MB, avg user read throughput: "
          << bytes_read * 1.0 / kMB / total_time << "MB/s" << endl;
  outFile << "user write size: " << bytes_write * 1.0 / kMB
          << "MB, avg user write throughput: "
          << bytes_write * 1.0 / kMB / total_time << "MB/s" << endl;
  outFile << "key write op: " << number_keys_write * 1.0 / 1000 / 1000
          << "Mop, avg key write ops: "
          << number_keys_write * 1.0 / total_time / 1000 << "kop/s" << endl;
  outFile << "key read op: " << number_keys_read * 1.0 / 1000 / 1000
          << "Mop, avg key read ops: "
          << number_keys_read * 1.0 / total_time / 1000 << "kop/s" << endl;
  outFile << "seek op: " << number_db_seek * 1.0 / 1000
          << "Kop, avg seek ops: " << number_db_seek * 1.0 / total_time
          << "kop/s" << endl;
  outFile << "next op: " << number_db_next * 1.0 / 1000
          << "Kop, avg next ops: " << number_db_next * 1.0 / total_time
          << "kop/s" << endl;
  outFile << "prev op: " << number_db_prev * 1.0 / 1000
          << "Kop, avg prev ops: " << number_db_prev * 1.0 / total_time
          << "kop/s" << endl;
  ...
  auto l4anduphit =
      db->GetOptions().statistics->getTickerCount(GET_HIT_L4_AND_UP);
  outFile << "Level hit status: memtable hit " << memhit << ", memtable miss "
          << memmiss << ", L0 " << l0hit << ", L1 " << l1hit << ", L2 "
          << l2hit << ", L3 " << l3hit << ", L4andup" << l4anduphit << endl;
...
void print_my_status(rocksdb::DB* db) {
  ...
  while (true) {
    ...
    auto disk_stats1 = get_disk_stats(target_disk_name1);
    // auto disk_stats_a1 = get_disk_stats(target_disk_name2);
    ...
    auto disk_stats2 = get_disk_stats(target_disk_name1);
    // auto disk_stats_a2 = get_disk_stats(target_disk_name2);

    outFile << "-------------------->\n";
    outFile << "MicroSecondsUp: " << db->GetOptions().SecondsUp() << endl;
    // 计算网络带宽
    ...
    // 计算磁盘带宽
    long long readDiff1 = disk_stats2.sectors_read - disk_stats1.sectors_read;
    long long writeDiff1 =
    ...
    outFile << "Disk Device: " << target_disk_name1 << ",";
    outFile << "Read Throughput: " << readThroughput / 1024.0 / 1024.0
            << " MB/s, ";  // 转换为 MB/s
    outFile << "Write Throughput: " << writeThroughput / 1024.0 / 1024.0
            << " MB/s\n";  // 转换为 MB/s

    // long long readDiff2 = disk_stats_a2.sectors_read - disk_stats_a1.sectors_read;
    // long long writeDiff2 =
    //     disk_stats_a2.sectors_written - disk_stats_a1.sectors_written;

    // // 每个 sector 一般为 512 字节
    // double readThroughput2 =
    //     (readDiff2 * 512) /
    //     static_cast<double>(intervalSeconds);  // 读取吞吐量 (字节/秒)
    // double writeThroughput2 =
    //     (writeDiff2 * 512) /
    //     static_cast<double>(intervalSeconds);  // 写入吞吐量 (字节/秒)

    // outFile << "Disk Device: " << target_disk_name2 << ",";
    // outFile << "Read Throughput: " << readThroughput2 / 1024.0 / 1024.0
    //         << " MB/s, ";  // 转换为 MB/s
    // outFile << "Write Throughput: " << writeThroughput2 / 1024.0 / 1024.0
    //         << " MB/s\n";  // 转换为 MB/s
    ...
    uint64_t number_keys_write_delta =
        number_keys_write - number_keys_write_snap;
    uint64_t number_keys_read_delta =
        number_keys_read - number_keys_read_snap;
    ...
    outFile << "write status:> total write op "
            << number_keys_write * 1.0 / 1000 / 1000
            << "Mops, interval write data size: "
            << bytes_write_delta * 1.0 / kMB << "MB, write throughput: "
            << bytes_write_delta * 1.0 / kMB / intervalSeconds
            << "MB/s, write ops: "
            << number_keys_write_delta * 1.0 / intervalSeconds << "op/s"
            << endl;

    outFile << "read status:> total read op "
            << number_keys_read * 1.0 / 1000 / 1000
            << "Mops, interval read data size: "
            << bytes_read_delta * 1.0 / kMB << "MB, read throughput: "
            << bytes_read_delta * 1.0 / kMB / intervalSeconds
            << "MB/s, read ops: "
            << number_keys_read_delta * 1.0 / intervalSeconds << "op/s"
            << endl;

    outFile << "iter status:> iter read size: "
            << iter_bytes_read_delta * 1.0 / kMB
            << "MB, iter read throughput: "
            << iter_bytes_read_delta * 1.0 / kMB / intervalSeconds
            << "MB/s, iter seek ops: "
            << number_db_seek_delta * 1.0 / intervalSeconds
            << "op/s, iter next ops: "
            << number_db_next_delta * 1.0 / intervalSeconds
            << "op/s, iter prev ops: " << number_db_prev_delta << "op/s"
            << endl;
    ...
    // add more statistics
    auto data_block_hit =
        db->GetOptions().statistics->getTickerCount(BLOCK_CACHE_DATA_HIT);
    auto data_block_miss =
        db->GetOptions().statistics->getTickerCount(BLOCK_CACHE_DATA_MISS);

    outFile << "BlockCache status: hit " << data_block_hit << ", miss "
            << data_block_miss << ", hit ritio "
            << data_block_hit * 1.0 / (data_block_hit + data_block_miss)
            << endl;

    // bloom filter
    auto useful =
        db->GetOptions().statistics->getTickerCount(BLOOM_FILTER_USEFUL);
    auto positive = db->GetOptions().statistics->getTickerCount(
        BLOOM_FILTER_FULL_POSITIVE);
    auto true_positive = db->GetOptions().statistics->getTickerCount(
        BLOOM_FILTER_FULL_TRUE_POSITIVE);
    auto false_positive = positive - true_positive;
    outFile << "BloomFilter status: useful " << useful << ", positive "
            << positive << ", true_positive " << true_positive
            << ", false_positive " << false_positive
            << " false positive ratio "
            << false_positive * 1.0 / (useful + positive) << endl;

    // S3 status
    std::string res;
    db->GetProperty("rocksdb.blob-stats",
                    &res);  // 修改了rocksdb内部rocksdb.blob-stats的打印信息
    outFile << res;
...
// Current the following isn't equivalent to OS_LINUX.
 ...
 public:
  ...
  void Run() {
    ...
    Open(&open_options_);
    my_stat_thr = std::thread(print_my_status, db_.db);
 ...
 private:
  ...
  void SetLocalOptions(rocksdb::Options* options) {
    options->create_if_missing = true;
    ...
    std::shared_ptr<const rocksdb::FilterPolicy> filter_policy(
        rocksdb::NewBloomFilterPolicy(10, 0));
  ...
  void SetOptions(Options* options) {
    ...
    block_based_options.block_size = 16 * 1024;
    block_based_options.initial_auto_readahead_size = 64 * 1024;
    block_based_options.max_auto_readahead_size = 2 * 1024 * 1024;
    options->table_factory.reset(
        rocksdb::NewBlockBasedTableFactory(block_based_options));
  }

  void SetLargeOptions(rocksdb::Options* options) {
    // L0和L1 256MB
    ...
    options->max_background_compactions = 8;
    options->max_background_flushes = 2;
    // options->soft_pending_compaction_bytes_limit = 16 * 1024 * 1024 * 1024ul;
    options->level0_file_num_compaction_trigger = 4;
    options->level0_slowdown_writes_trigger = 20;
    options->level0_stop_writes_trigger = 36;
  ...
  enum Layout { ALL_S3, Hyper, ALL_EBS };
  void SetSpecialOptions(rocksdb::Options* options, std::string& kDBPath, enum Layout layout) {
    if (layout == Layout::ALL_S3) {
      ...
      options->db_paths = {
          {kDBPath + "/ebs", 1024l * 1024 * 1024 * 1024},
          {kDBPath + "/s3", 1024l * 1024 * 1024 * 1024},
          // {"/home/ubuntu/gpa/s3", 1024l * 1024 * 1024 * 1024},
      };
  ...
  void OpenDb(Options options, const std::string& db_name,
    ...
    if (FLAGS_use_hyper_cloud_db) { 
      ...
      std::string MY_LOCAL_DBPATH = "/tmp/dbbench_fillrandom";
      std::string MY_CLOUD_DBPATH = "/home/ubuntu/gp3/dbbench_fillrandom";
      std::string kDBPath = MY_CLOUD_DBPATH;
      std::string kBucketSuffix = "dbbench.fillrandom";
      ...
      // open DB
      DBCloud* cloud_db;
      s = DBCloud::Open(options, kDBPath, persistent_cache, 0, &cloud_db);
      if (!s.ok()) {
