    ...
    for (int c = 0; c < 256; ++c) {
      ...
      for (char i = 0; i < 8; ++i) {
        if ((c >> i) & 1) {  // opening
          ...
          --excess;
          if (excess < 0 && m_fwd_pos[c][-excess] == 0) {  // not already found
...
inline bool find_close_in_word(uint64_t word, uint64_t byte_counts,
                               bp_vector::excess_t cur_exc, uint64_t& ret) {
  assert(cur_exc > 0 && cur_exc <= 64);
  const uint64_t cum_exc_step_8 =
      (cur_exc + ((2 * byte_counts - 8 * broadword::ones_step_8) << 8)) *
      broadword::ones_step_8;

  uint64_t min_exc_step_8 = 0;
  for (int i = 0; i < 8; ++i) {
    size_t shift = i * 8;
    min_exc_step_8 |= ((uint64_t)(tables.m_fwd_min[(word >> shift) & 0xFF]))
                      << shift;
  }

  const uint64_t has_result =
      broadword::leq_step_8(cum_exc_step_8, min_exc_step_8);

  unsigned long shift;
  if (broadword::lsb(has_result, shift)) {
    uint8_t bit_pos = tables.m_fwd_pos[(word >> shift) & 0xFF]
                                      [(cum_exc_step_8 >> shift) & 0xFF];
    assert(bit_pos > 0);
    ret = shift + bit_pos - 1;
    return true;
  }
...
inline bool find_open_in_word(uint64_t word, uint64_t byte_counts,
                              bp_vector::excess_t cur_exc, uint64_t& ret) {
  assert(cur_exc > 0 && cur_exc <= 64);
  const uint64_t rev_byte_counts = broadword::reverse_bytes(byte_counts);
  const uint64_t cum_exc_step_8 =
      (cur_exc - ((2 * rev_byte_counts - 8 * broadword::ones_step_8) << 8)) *
      broadword::ones_step_8;
  ...
  for (int i = 0; i < 8; ++i) {
    size_t shift = i * 8;
    max_exc_step_8 |=
        ((uint64_t)(tables.m_bwd_min[(word >> (64 - shift - 8)) & 0xFF]))
        << shift;
  }

  const uint64_t has_result =
      broadword::leq_step_8(cum_exc_step_8, max_exc_step_8);

  unsigned long shift;
  if (broadword::lsb(has_result, shift)) {
    uint8_t bit_pos = tables.m_bwd_pos[(word >> (64 - shift - 8)) & 0xFF]
                                      [(cum_exc_step_8 >> shift) & 0xFF];
    assert(bit_pos > 0);
...
inline void excess_rmq_in_word(uint64_t word, bp_vector::excess_t& exc,
                               uint64_t word_start,
                               bp_vector::excess_t& min_exc,
                               uint64_t& min_exc_idx) {
  ...
  if (min_byte_exc < min_exc) {
    ...
    min_exc_idx =
        word_start + shift + tables.m_fwd_min_idx[(word >> shift) & 0xFF];
  }
}
}

    inline bool bp_vector::find_close_in_block(uint64_t block_offset,
                                           bp_vector::excess_t excess,
                                           uint64_t start,
                                           uint64_t& ret) const {
  ...
  assert(excess > 0);
  for (uint64_t sub_block_offset = start; sub_block_offset < bp_block_size;
       ++sub_block_offset) {
...
uint64_t bp_vector::find_close(uint64_t pos) const {
  ...
  uint64_t local_rank =
      broadword::bytes_sum(byte_counts) - shift;  // subtract back the padding
  excess_t local_excess =
      static_cast<excess_t>((2 * local_rank) - (64 - shift));
  ...
  bool found = find_close_in_block(found_block_offset,
                                   found_block_excess - pos_excess, 0, ret);
  assert(found);
  (void)found;
  return ret;
}

inline bool bp_vector::find_open_in_block(uint64_t block_offset,
                                          bp_vector::excess_t excess,
  ...
  for (uint64_t sub_block_offset = start - 1; sub_block_offset + 1 > 0;
       --sub_block_offset) {
    assert(excess > 0);
    uint64_t sub_block = block_offset + sub_block_offset;
    uint64_t word = m_bits[sub_block];
...
uint64_t bp_vector::find_open(uint64_t pos) const {
  ...
  uint64_t sub_block = word_pos % bp_block_size;
  uint64_t local_rank =
      broadword::bytes_sum(byte_counts);  // no need to subtract the padding
  ...
  excess_t found_block_excess = 0;
  found_block_excess = get_block_excess(found_block + 1);

  // Search in the found block
  bool found = find_open_in_block(
      found_block_offset, found_block_excess - pos_excess, bp_block_size, ret);
  assert(found);
  (void)found;
  return ret;
}
...
inline bool bp_vector::search_block_in_superblock(uint64_t block,
                                                  excess_t excess,
                                                  size_t& found_block) const {
  ...
  if (direction) {
    for (size_t cur_block = block;
         cur_block < std::min((superblock + 1) * superblock_size,
                              (size_t)m_block_excess_min.size());
         ++cur_block) {
    ...
    for (size_t cur_block = block;
         cur_block + 1 >= (superblock * superblock_size) + 1; --cur_block) {
      if (excess >= superblock_excess + m_block_excess_min[cur_block]) {
        found_block = cur_block;
        return true;
      }
...
inline bp_vector::excess_t bp_vector::get_block_excess(uint64_t block) const {
  ...
  excess_t excess =
      static_cast<excess_t>(2 * sub_block_rank(sub_block_idx) - block_pos);
  assert(excess >= 0);
  return excess;
}
...
template <int direction>
inline uint64_t bp_vector::search_min_tree(uint64_t block,
                                           excess_t excess) const {
  size_t found_block;
  ...
  bool ret = search_block_in_superblock<direction>(
      next_superblock * superblock_size +
          (1 - direction) * (superblock_size - 1),
      excess, found_block);
  assert(ret);
  (void)ret;
...
bp_vector::excess_t bp_vector::excess(uint64_t pos) const {
  return static_cast<excess_t>(2 * rank(pos) - pos);
}

void bp_vector::excess_rmq_in_block(uint64_t start, uint64_t end,
  ...
  for (size_t w = start; w < end; ++w) {
    excess_rmq_in_word(m_bits[w], exc, w * 64, min_exc, min_exc_idx);
  }
}

void bp_vector::excess_rmq_in_superblock(uint64_t block_start,
                                         uint64_t block_end,
...
void bp_vector::find_min_superblock(uint64_t superblock_start,
                                    uint64_t superblock_end,
...
uint64_t bp_vector::excess_rmq(uint64_t a, uint64_t b,
                               excess_t& min_exc) const {
  assert(a <= b);
  ...
  uint64_t padded_word_a = (subword_len_a == 64)
                               ? shifted_word_a
                               : (shifted_word_a | (~0ULL << subword_len_a));

  excess_rmq_in_word(padded_word_a, cur_exc, a, min_exc, min_exc_idx);
  ...
  if (block_a == block_b) {
    // same block
    excess_rmq_in_block(word_a_idx + 1, word_b_idx, cur_exc, min_exc,
                        min_exc_idx);

  } else {
    // search in partial block of word_a
    excess_rmq_in_block(word_a_idx + 1, (block_a + 1) * bp_block_size, cur_exc,
                        min_exc, min_exc_idx);
    ...
    if (superblock_a == superblock_b) {
      // same superblock
      excess_rmq_in_superblock(block_a + 1, block_b, block_min_exc,
                               block_min_idx);
    } else {
      ...
      find_min_superblock(superblock_a + 1, superblock_b, superblock_min_exc,
                          superblock_min_idx);
      ...
      excess_rmq_in_superblock(superblock_b * superblock_size, block_b,
                               block_min_exc, block_min_idx);
    }
    ...
    if (block_min_exc < min_exc) {
      ...
      excess_rmq_in_block(block_min_idx * bp_block_size,
                          (block_min_idx + 1) * bp_block_size, cur_exc, min_exc,
                          min_exc_idx);
    ...
    excess_rmq_in_block(block_b * bp_block_size, word_b_idx, cur_exc, min_exc,
                        min_exc_idx);
  }
  ...
  uint64_t padded_word_b =
      (offset_b == 0) ? word_b : (word_b | (~0ULL << offset_b));

  excess_rmq_in_word(padded_word_b, cur_exc, word_b_idx * 64, min_exc,
                     min_exc_idx);
...
void bp_vector::build_min_tree() {
  ...
  for (uint64_t sub_block = 0; sub_block < m_bits.size(); ++sub_block) {
    ...
    uint64_t n_bits =
        (sub_block == m_bits.size() - 1 && size() % 64) ? size() % 64 : 64;
  ...
  while (n_complete_leaves < n_superblocks)
    n_complete_leaves <<=
        1;  // XXX(ot): I'm sure this can be done with broadword::msb...
  ...
  for (size_t superblock = 0; superblock < n_superblocks; ++superblock) {
    ...
    for (size_t block = superblock * superblock_size;
         ...
         ++block) {
      cur_super_min =
          std::min(cur_super_min, superblock_excess + block_excess_min[block]);
    }
  ...
  for (size_t node = treesize - 1; node > 1; --node) {
    ...
    superblock_excess_min[parent] =
        std::min(superblock_excess_min[parent],  // same node
